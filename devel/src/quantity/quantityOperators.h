...
constexpr
Quantity<typename QSrc::unit, TDst>
static_unit_cast(QSrc const& l)
{
...
constexpr Quantity<U, LT>& operator+=( Quantity<U, LT> & l, Quantity<U, RT> const & r ){ return l.m_magnitude += r.m_magnitude, l;}
...
{
    return Quantity<product_unit<LU,RU>, decltype(l.magnitude()*r.magnitude())> {
...
{
    return Quantity<quotient_unit<LU,RU>, decltype(l.magnitude()/r.magnitude())> {
                                                   l.magnitude()/r.magnitude()};
}
...
{
    return Quantity<U, decltype(l.magnitude()+r.magnitude())> {
                                l.magnitude()+r.magnitude()};
}
