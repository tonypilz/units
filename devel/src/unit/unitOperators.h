...
struct multiply{
      static_assert(helper::exponent<U1,index>() * fract::num % fract::den == 0 ,"rasing a unit to a fractional-power is not allowed");
...
using product_unit = typename helper::UnitOperator<U1::exponent_count()-1,U1,U2,helper::sum>::type;

template<typename U1,typename U2>
using quotient_unit = typename helper::UnitOperator<U1::exponent_count()-1,U1,U2,helper::difference>::type;

template<typename U1, typename fract>
using raised_unit = typename helper::UnitOperator<U1::exponent_count()-1,U1,fract,helper::multiply>::type;
